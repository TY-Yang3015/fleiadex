...
class VanillaUNet2D(nn.Module):
    ...
    def setup(self):
        ...
        self.input_conv_proj = nn.Conv(features=self.base_channels,
                                       kernel_size=(7, 7),
                                       strides=(1, 1),
                                       padding='SAME')
        ...
        self.time_embedding = TimeEmbedding(output_channels=self.base_channels,
                                            dropout=0)
        ...
        if self.merge_temporal_dim_and_image_channels:
            current_channels = (self.sample_input_shape[0] + self.cond_input_shape[0]) * self.base_channels
        else:
        ...
        for _, down_sample_factor in enumerate(self.spatial_down_sample_schedule):
            ...
            if self.down_sample_use_attention:
                down_sample_attention_block.append(SelfAttention(
                    output_channels=current_channels,
            ...
            if self.down_sampler_type == 'conv':
                ...
                down_samplers.append(nn.Conv(features=current_channels,
                                             kernel_size=(3, 3),
                                             strides=(down_sample_factor, down_sample_factor),
                                             padding='SAME'))
            elif self.down_sampler_type == 'patch_merge':
                down_samplers.append(PatchMerge3D(
                ...
                raise NotImplementedError('only "conv" and "patch_merge" are supported for down-sampling.')
        ...
        for _, down_sample_factor in enumerate(reversed(self.spatial_down_sample_schedule)):
            ...
            if self.up_sample_use_attention:
                up_sample_attention_block.append(SelfAttention(
                    output_channels=current_channels,
            ...
            if self.up_sampler_type == 'conv_transpose':
                ...
                up_samplers.append(nn.ConvTranspose(features=current_channels,
                                                    kernel_size=(3, 3),
                                                    strides=(down_sample_factor, down_sample_factor),
                                                    padding='SAME'))
            elif self.up_sampler_type == 'interp2d':
                if self.merge_temporal_dim_and_image_channels:
                    up_samplers.append(UpSampler2D(
                        output_channels=int(current_channels / down_sample_factor ** 2),
                        target_size=(latent_size * down_sample_factor,
                                     latent_size * down_sample_factor)
                    ))
                else:
                    up_samplers.append(UpSampler3D(
                        output_channels=int(current_channels / down_sample_factor ** 2),
                ...
                raise NotImplementedError('only "conv_transpose" and "interp2d" are supported for up-sampling.')
