...
class DDPMManager(nn.Module):
    ...
    def setup(self) -> None:
        ...
        self.alphas_cumprod_prev = jnp.append(1., self.alpha_cumprod[:-1])

        self.sqrt_alphas_cumprod = jnp.sqrt(self.alpha_cumprod)
        self.sqrt_one_minus_alphas_cumprod = jnp.sqrt(1. - self.alpha_cumprod)

        self.log_one_minus_alphas_cumprod = jnp.log(1. - self.alpha_cumprod)
        self.sqrt_recip_alphas_cumprod = jnp.sqrt(1 / self.alpha_cumprod)
        self.sqrt_recipm1_alphas_cumprod = jnp.sqrt(1. / self.alpha_cumprod - 1.)

        self.posterior_var = self.betas * (1. - self.alphas_cumprod_prev) / (1. - self.alpha_cumprod)

        self.posterior_log_variance_clipped = jnp.log(jnp.maximum(self.posterior_var, 1e-20))

        self.posterior_mean_coef1 = self.betas * jnp.sqrt(self.alphas_cumprod_prev) / (1. - self.alpha_cumprod)

        self.posterior_mean_coef2 = (1. - self.alphas_cumprod_prev) * jnp.sqrt(self.alphas) / (1. - self.alpha_cumprod)
