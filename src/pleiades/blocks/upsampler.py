...
class UpSampler(nn.Module):
    ...
    def setup(self):
        self.conv = nn.Conv(features=self.output_channels,
                            strides=(1, 1),
                            kernel_size=(self.kernel_size, self.kernel_size),
                            padding=(self.kernel_size // 2, self.kernel_size // 2),
                            kernel_init=nn.initializers.kaiming_normal())
        if self.layout not in ['t h w c', 'c t h w']:
            raise ValueError(f'unsupported layout: {self.layout}, please choose one of the '
    ...
    def __call__(self, x):
        if self.layout == 't h w c':
            ...
            if self.temporal_upsample:
                x = jax.image.resize(x, (batch_size,) + self.target_size + (c,),
                                     method='nearest')
                x = self.conv(x)
                return x
            else:
                if t != self.target_size[0]:
                    raise ValueError('if temporal_upsample is False, the target size must '
