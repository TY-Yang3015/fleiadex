...
class TimeEmbeddingInit(nn.Module):
    ...
    @nn.compact
    def __call__(
        self, time_stamps: jnp.ndarray, max_period: int = 10000, is_repeat: bool = False
    ) -> jnp.ndarray:
        time_embedding = self.timestep_sinusoidal_embedding(
            time_stamps, self.time_embedding_channels, max_period, is_repeat
        )
        time_embedding = nn.Dense(
            self.time_embedding_channels,
            kernel_init=nn.initializers.kaiming_normal(),
            bias_init=nn.initializers.zeros_init(),
            name=f"time_embedding_dense1",
        )(time_embedding)
        ...
        time_embedding = nn.Dense(
            ...
            kernel_init=nn.initializers.kaiming_normal(),
            bias_init=nn.initializers.zeros_init(),
            name=f"time_embedding_dense2",
        )(time_embedding)
        return time_embedding

    def timestep_sinusoidal_embedding(
        self,
        time_stamp: jnp.ndarray,
        ...
        if not is_repeat:
            ...
            embedding = jnp.concatenate(
                [jnp.cos(arguments), jnp.sin(arguments)], axis=-1
            )
            if sinusoidal_embedding_dimensions % 2:
                embedding = jnp.concatenate(
                    [jnp.sin(arguments), jnp.zeros_like(embedding[:, :1])], axis=-1
                )
        else:
            embedding = repeat(
                time_stamp, "b -> b d", d=sinusoidal_embedding_dimensions
            )
...
class TimeEmbedding(nn.Module):
    """
    the time embedding layer with residual connection at the end, **require the same input and output shapes**.
    if not the case, use ``TimeEmbeddingResBlock()`` instead.
...
class TimeEmbeddingResBlock(nn.Module):
    ...
    def __call__(self, x: jnp.ndarray, emb: jnp.ndarray, train: bool) -> jnp.ndarray:
        ...
        emb = rearrange(emb, "b c -> b 1 1 1 c")
        if x.ndim == 4:
            emb = rearrange(emb, "b 1 1 1 c -> b 1 1 c")
        h = emb + x_skip
