...
from fleiadex.errors import StructureError
from fleiadex.blocks.utils import pad_input, unpad_output
...
class CuboidSelfAttention(nn.Module):
    ...
    def setup(self):
        ...
        if self.use_relative_position:
            self.relative_position_bias_table = self.param(
                ...
                (
                    jnp.int32(
                        (2 * self.cuboid_size[0] - 1)
                        * (2 * self.cuboid_size[1] - 1)
        ...
        if self.use_final_projector:
            self.final_projector = FinalProjection(
                self.input_channels, self.final_projection_dropout
            )
    ...
    def _get_relative_position_index(self) -> jnp.ndarray:
        ...
        relative_coords = rearrange(coords, "b thw -> b thw 1") - rearrange(
            coords, "b thw -> b 1 thw"
        )
        relative_coords = rearrange(relative_coords, "b thw1 thw2 -> thw1 thw2 b")

        relative_coords.at[:, :, 0].set(
            relative_coords[:, :, 0] + self.cuboid_size[0] - 1
        )
        relative_coords.at[:, :, 1].set(
            relative_coords[:, :, 1] + self.cuboid_size[1] - 1
        )
        relative_coords.at[:, :, 2].set(
    ...
    def _cuboid_reorder(self, x: jnp.ndarray, cuboid_size, strategy) -> jnp.ndarray:
        ...
        number_of_cuboid = (
            t // cuboid_size[0] * h // cuboid_size[1] * w // cuboid_size[2]
        )
        ...
        n_block_axis = []
        block_axis = []
        for i, (block_size, total_size, element_strategy) in enumerate(
            zip(cuboid_size, (t, h, w), strategy)
        ):
            if element_strategy == "l":
                intermediate_shape.extend(
                    [int(total_size // block_size), int(block_size)]
                )
                n_block_axis.append(2 * i + 1)
                block_axis.append(2 * i + 2)
            elif element_strategy == "d":
                intermediate_shape.extend(
                    [int(block_size), int(total_size // block_size)]
                )
                n_block_axis.append(2 * i + 2)
                block_axis.append(2 * i + 1)
            else:
                NotImplementedError(
                    f"the elementwise strategy {element_strategy} is not implemented.\
                use 'l' or 'd' instead."
                )
    ...
    def _reverse_cuboid_reorder(
        self, x: jnp.ndarray, cuboid_size, strategy, original_shape
    ) -> jnp.ndarray:
    ...
    def _compute_attention_mask(
        self, pads, shape, size, shift, strategy, padding_type
    ) -> jnp.ndarray:
        ...
        if any(i > 0 for i in shift):
            if padding_type == "auto":
                data_mask = jnp.roll(
                    data_mask, shift=(-shift[0], -shift[1], -shift[2]), axis=(1, 2, 3)
                )
        if padding_type == "auto":
        ...
        for ti in slice(-size[0]), slice(-size[0], -shift[0]), slice(-shift[0], None):
            for hi in (
                slice(-size[1]),
                slice(-size[1], -shift[1]),
                slice(-shift[1], None),
            ):
                for wi in (
                    slice(-size[2]),
                    slice(-size[2], -shift[2]),
                    slice(-shift[2], None),
                ):
        ...
        attention_mask = (
            jnp.expand_dims(shift_mask, axis=1) - jnp.expand_dims(shift_mask, axis=2)
            == 0
        )
        if padding_type == "auto":
            attention_mask = (
                jnp.expand_dims(data_mask, axis=1)
                * jnp.expand_dims(data_mask, axis=2)
                * attention_mask
            )
    ...
    def masked_softmax(self, attention_score, mask, axis=-1):
        ...
        if mask is not None:
            mask = jnp.tile(
                mask, (attention_score.shape[0], attention_score.shape[1], 1, 1, 1)
            )
    ...
    def __call__(self, x_input: jnp.ndarray, train: bool) -> jnp.ndarray:
        ...
        assert c_in == self.input_channels, (
            "the input channel dimension "
            "does not much the specified dimension value."
        )
        assert c_in % self.attention_heads == 0, (
            "the attention head dimension must " "be a factor of the input dimension."
        )
        ...
        x_reordered = self._cuboid_reorder(
            shifted_x, cuboid_size=self.cuboid_size, strategy=self.strategy
        )
        ...
        qkv = self.qkv(x_reordered).reshape(
            batch_size,
            number_of_cuboids,
            cuboid_volume,
            3,
        ...
        q = (
            q * jnp.float32(self.input_channels // self.attention_heads) ** -0.5
        )  # scale q
        ...
        if self.use_relative_position:
            ...
            relative_position_bias = rearrange(
                relative_position_bias, "cv1 cv2 heads -> heads 1 cv1 cv2"
            )
        ...
        x_reordered = x_reordered.reshape(
            batch_size, number_of_cuboids, cuboid_volume, c_in
        )

        if self.use_final_projector:
            x_reordered = self.final_projector(x_reordered, train)

        x_output = self._reverse_cuboid_reorder(
            x_reordered,
            self.cuboid_size,
            self.strategy,
