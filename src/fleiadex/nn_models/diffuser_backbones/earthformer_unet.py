...
from fleiadex.nn_models.diffuser_backbones.default_factories import DefaultFactory
...
class EarthformerUNet(nn.Module):
    ...
    base_units: int = 128
    io_depth: list[int] = field(default_factory=DefaultFactory.default_depth)
    bottleneck_depth: int = 8
    spatial_downsample_factor: int = 2
    upsampler_kernel_size: int = 3
    block_attention_pattern: str = "axial"
    block_cuboid_sizes: list[tuple[int, int, int]] = field(
        default_factory=DefaultFactory.default_block_cuboid_sizes
    )
    block_cuboid_strategies: list[tuple[str, str, str]] = field(
        default_factory=DefaultFactory.default_block_cuboid_strategy
    )
    ...
    time_embedding_dropout_rate: float = 0.0

    dtype: jnp.dtype = jnp.float32

    def setup(self) -> None:

        # self.block_cuboid_sizes = ((4, 4, 4), (4, 4, 4))
        ...
        self.init_time_embedding = TimeEmbeddingInit(
            time_embedding_channels=self.base_units
            * self.time_embedding_channels_multiplier
        )
    ...
    def __call__(
        ...
        if (
            (x.shape[1:] != self.sample_input_shape)
            or (cond.shape[1:] != self.cond_input_shape)
