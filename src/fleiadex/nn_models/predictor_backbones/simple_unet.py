...
class SimpleUNet(nn.Module):
    ...
    def __call__(self, x, training: bool):
        ...
        for i in range(self.layers):
            z = nn.Conv(
                self.features * 2 ** (self.layers - i),
                kernel_size=(4, 4),
                kernel_init=jax.nn.initializers.glorot_uniform(),
                input_dilation=(2, 2),
                padding=((1, self.ks - 1), (1, self.ks - 1)),
            )(z)
            z = jnp.concatenate((zs[self.layers - i - 1], z), axis=3)
            z = nn.Conv(
                self.features * 2 ** (self.layers - i - 1),
                kernel_size=(4, 4),
                kernel_init=jax.nn.initializers.glorot_uniform(),
            )(z)
            z = nn.GroupNorm(num_groups=8)(z)
        ...
        # output layer
        z_conv = nn.Conv(
            1, kernel_size=(1, 1), kernel_init=jax.nn.initializers.glorot_uniform()
        )(z)
